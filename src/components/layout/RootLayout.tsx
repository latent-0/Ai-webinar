...
import { Radio, Brain, Gamepad2, Zap } from 'lucide-react'

const navLinks = [
  { to: '/live',  label: 'Live',      icon: Radio,    dot: true  },
  { to: '/learn', label: 'Assistant', icon: Brain,    dot: false },
  { to: '/play',  label: 'Sandbox',   icon: Gamepad2, dot: false },
]
            ...
            return (
              <Link key={to} to={to}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  isActive ? 'bg-indigo-50 text-indigo-700' : 'text-[#6B7280] hover:bg-gray-100 hover:text-[#111827]'
                }`}>
                {dot && (
